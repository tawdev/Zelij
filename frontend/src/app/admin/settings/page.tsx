...
export default function AdminSettingsPage() {
    ...
    const loadSettings = useCallback(async () => {
        try {
            ...
            if (data.logoDarkUrl && data.logoDarkUrl.startsWith('/')) {
                const apiBase = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:3002';
                data.logoDarkUrl = `${apiBase}${data.logoDarkUrl}`;
            }
    ...
    const handleLogoDarkChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setSettings(prev => ({ ...prev, logoDarkUrl: absoluteLogoUrl }));
            showToast('Logo sombre téléchargé. Cliquez sur Enregistrer.', 'info');
        } catch (err) {
            showToast('Échec du téléchargement du logo sombre', 'error');
        } finally {
            setIsUploading(false);
        }
    };
    ...
    const handleRemoveLogoDark = () => {
        ...
        showToast('Logo sombre retiré.', 'info');
    };
                ...
                <div className="space-y-6 animate-in fade-in duration-500">
                    ...
                    <div className="bg-white p-8 rounded-xl border border-slate-200 shadow-sm">
                        ...
                        <div className="space-y-6">
                            ...
                            <div className="space-y-2 pt-2">
                                <label className="text-[13px] font-bold text-slate-600">Working Hours</label>
                                <textarea
                                    value={settings.workingHours || ''}
                                    onChange={(e) => setSettings({ ...settings, workingHours: e.target.value })}
                                    rows={2}
                                    placeholder="Ex: Lun – Sam : 9h00 – 20h00, Dimanche : Fermé"
                                    className="w-full p-4 rounded-xl bg-white border border-slate-200 text-[14px] font-medium text-slate-800 focus:outline-none focus:ring-1 focus:ring-primary/20 focus:border-primary transition-all resize-none"
                                />
                            </div>

                            <div className="pt-8 mb-4 border-t border-slate-100">
                                <h4 className="text-[13px] font-bold text-slate-600 mb-4">Logo Principal (Site Web)</h4>
                                    ...
                                    <div className="space-y-4 pt-4">
                                        <div className="flex flex-wrap items-center gap-3">
                                            <div className="relative">
                                                ...
                                                >
                                                    {isUploading ? 'Uploading...' : 'Changer Logo'}
                                                </button>
                                                <input 
                                                    ref={fileInputRef}
                                                    type="file" 
                                        ...
                                        <p className="text-[12px] font-medium text-slate-400">Pour le thème sombre du site.</p>
                                    </div>
                                </div>
                            </div>

                            <div className="pt-8 mb-4 border-t border-slate-100">
                                <h4 className="text-[13px] font-bold text-slate-600 mb-4">Logo pour Documents (Version Sombre)</h4>
                                <div className="flex items-start gap-4">
                                    <div className="size-[88px] rounded-xl bg-white flex items-center justify-center border border-slate-200 overflow-hidden shrink-0 mt-1 relative group">
                                        {settings.logoDarkUrl ? (
                                            <img src={settings.logoDarkUrl} alt="Store Logo Dark" className="w-full h-full object-cover" />
                                        ) : (
                                            <div className="flex flex-col items-center opacity-20">
                                                <span className="material-symbols-outlined text-black text-2xl mb-1">spa</span>
                                                <span className="text-[6px] font-bold text-black tracking-wider uppercase">Droguerie</span>
                                            </div>
                                        )}
                                        {isUploading && (
                                    ...
                                    <div className="space-y-4 pt-4">
                                        <div className="flex flex-wrap items-center gap-3">
                                            <div className="relative">
                                                ...
                                                <button 
                                                    ...
                                                    disabled={isUploading}
                                                    className="h-9 px-6 rounded-lg bg-slate-900 hover:bg-black text-white text-[13px] font-bold transition-opacity disabled:opacity-50"
                                                >
                                                    {isUploading ? 'Uploading...' : 'Changer Logo Sombre'}
                                                </button>
                                            </div>
                                            {settings.logoDarkUrl && (
                                                <button 
                                        ...
                                        <p className="text-[12px] font-medium text-slate-400">À utiliser pour les devis et documents PDF (fond blanc).</p>
                                    </div>
                                </div>
                            </div>
                        </div>

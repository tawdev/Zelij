...
export default function AdminOrdersPage() {
  ...
  const [deleteModalConfig, setDeleteModalConfig] = useState<{ type: 'single' | 'bulk'; id?: number }>({ type: 'single' });
  ...
  const loadData = useCallback(async () => {
    try {
      ...
      const [s, o] = await Promise.all([
        api.getOrderStats(),
        api.getOrders(page, 20, statusFilter || undefined, debouncedSearch || undefined),
      ]);
      setStats(s);
      setOrders(o.data);
      setTotal(o.total);
  ...
  const handleBulkDelete = async () => {
    setDeleteModalConfig({ type: 'bulk' });
    setIsDeleteModalOpen(true);
  };
  ...
  const confirmDelete = async () => {
    try {
      if (deleteModalConfig.type === 'single' && deleteModalConfig.id) {
        setActionLoading(deleteModalConfig.id);
        await api.deleteOrder(deleteModalConfig.id);
        showToast('Commande supprimée avec succès.', 'success');
      } else if (deleteModalConfig.type === 'bulk') {
        setIsBulkDeleting(true);
        await api.bulkDeleteOrders(selectedIds);
        showToast(`${selectedIds.length} commandes supprimées avec succès.`, 'success');
        setSelectedIds([]);
      }
  ...
  return (
    <main className="flex-1 overflow-y-auto no-scrollbar p-4 md:p-8 bg-white">
      ...
      <div className="bg-white rounded-[32px] border border-slate-100 shadow-sm overflow-hidden">
          ...
          {selectedIds.length > 0 && (
            <div className="ml-auto flex items-center gap-3">
              ...
              <button
                ...
                className="flex items-center gap-2 px-4 py-2 bg-rose-600 text-white rounded-xl text-[10px] font-black uppercase tracking-widest hover:bg-rose-700 transition-all shadow-lg shadow-rose-200 disabled:opacity-50"
              >
                <Trash2 size={14} />
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-slate-100">
                <th className="px-6 py-5 text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] w-10">
                  <button onClick={toggleSelectAll} className="text-slate-400 hover:text-slate-900 transition-all">
                    {selectedIds.length === orders.length && orders.length > 0 ? <CheckSquare size={18} className="text-primary" /> : <Square size={18} />}
                  </button>
                </th>
            ...
            <tbody className="divide-y divide-slate-50">
              {loading && orders.length === 0 ? (
                [...Array(5)].map((_, i) => (
                  <tr key={i}>
                    <td className="px-6 py-6 w-10"><Skeleton className="h-5 w-5 rounded" /></td>
                ...
                orders.map((order) => (
                  <React.Fragment key={order.id}>
                    <tr className={`hover:bg-slate-50/50 transition-all cursor-pointer ${expandedOrder === order.id ? 'bg-slate-50/50' : ''} ${selectedIds.includes(order.id) ? 'bg-primary/5' : ''}`} onClick={() => setExpandedOrder(expandedOrder === order.id ? null : order.id)}>
                      <td className="px-6 py-6 w-10" onClick={(e) => e.stopPropagation()}>
                        <button onClick={() => toggleSelectOne(order.id)} className="text-slate-300 hover:text-slate-900 transition-all">
                          {selectedIds.includes(order.id) ? <CheckSquare size={18} className="text-primary" /> : <Square size={18} />}
                        </button>
                      </td>
                      ...
                      <td className="px-6 py-6 text-right">
                        <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                          ...
                          >
                            <Trash2 size={16} className={actionLoading === order.id ? 'animate-pulse' : ''} />
                          </button>
                          <button
                              ...
                              <div className="p-4 bg-white rounded-2xl border border-slate-100 flex items-center justify-between shadow-sm">
                                ...
                                <div className="flex gap-2">
                                  <button
                                    ...
                                    className="p-2 text-slate-400 hover:text-primary transition-all flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest border border-slate-100 rounded-lg"
        ...
        <div className="p-6 bg-slate-50/30 border-t border-slate-100 dark:border-slate-800 flex items-center justify-between">
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400">
            {loading ? 'Chargement...' : `Affichage ${orders.length > 0 ? (page - 1) * 20 + 1 : 0}–${Math.min(page * 20, total)} sur ${total.toLocaleString()}`}
          </p>
          <div className="flex gap-2">
            <button
      ...
      <DeleteConfirmModal
        ...
        loading={actionLoading !== null || isBulkDeleting}
        title={deleteModalConfig.type === 'bulk' ? 'Suppression groupée' : 'Supprimer la commande'}
        message={deleteModalConfig.type === 'bulk' 
          ? `Êtes-vous sûr de vouloir supprimer les ${selectedIds.length} commandes sélectionnées ? Cette action est irréversible.` 
      ...
      {selectedIds.length > 0 && (
        <div className="fixed bottom-8 left-1/2 -translate-x-1/2 z-[90] animate-in slide-in-from-bottom-10 duration-300">
          <div className="bg-slate-900 text-white px-6 py-4 rounded-[24px] shadow-2xl flex items-center gap-6 backdrop-blur-md bg-opacity-95 border border-white/10">
            <div className="flex items-center gap-3 pr-6 border-r border-white/10">
              <div className="bg-primary p-2 rounded-lg text-white">
                <CheckSquare size={18} />
              </div>
              <div>
                <p className="text-[10px] font-black uppercase tracking-widest text-white/50 leading-none mb-1">Sélection</p>
                <p className="text-sm font-black tracking-tight leading-none">{selectedIds.length} commandes</p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              <button
                ...
                className="px-4 py-2 text-[10px] font-black uppercase tracking-widest text-white/70 hover:text-white transition-all"
              >
                Désélectionner
              </button>
              <button
                onClick={handleBulkDelete}
                disabled={isBulkDeleting}
                className="flex items-center gap-2 px-6 py-2.5 bg-rose-600 hover:bg-rose-700 text-white rounded-xl text-[10px] font-black uppercase tracking-widest transition-all shadow-lg shadow-rose-900/20 disabled:opacity-50"
              >
                <Trash2 size={14} />
                Supprimer
              </button>

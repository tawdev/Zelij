...
export default function ProductRating({ 
    ...
    useEffect(() => {
        ...
        if (initialAvg !== undefined && initialCount !== undefined) {
            setStats({ avg: initialAvg, count: initialCount });
        }
    ...
    useEffect(() => {
        ...
        // Only try to load from local storage if we don't have props or if we want to allow local updates
        const handleReviewUpdate = () => loadLocalStats();
        ...
        window.addEventListener('storage', (e) => {
            if (e.key === `reviews_${productId}`) handleReviewUpdate();
        });

                        ...
                        {searchResults.orders.map(order => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px] text-slate-500 truncate">#{String(order.id).padStart(4, '0')} • {Number(order.totalPrice).toFixed(2)} MAD</p>
                            </div>
                        ...
                        {searchResults.products.map(product => (
                          ...
                          >
                            ...
                            <div className="flex-1 min-w-0">
                              ...
                              <p className="text-[10px] text-slate-500 truncate">{product.category?.name || 'Uncategorized'} • {Number(product.price).toFixed(2)} MAD</p>
                            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-3">
            ...
            <div>
              ...
              {loading ? <Skeleton className="h-8 w-20 mt-1" /> : <p className="text-slate-900 text-2xl font-bold mt-1">{productStats?.total.toLocaleString() ?? '—'}</p>}
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-3">
            ...
            <div>
              ...
              {loading ? <Skeleton className="h-8 w-20 mt-1" /> : <p className="text-slate-900 text-2xl font-bold mt-1">{orderStats?.total.toLocaleString() ?? '—'}</p>}
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-3">
            ...
            <div>
              ...
              {loading ? <Skeleton className="h-8 w-20 mt-1" /> : <p className="text-slate-900 text-2xl font-bold mt-1">{orderStats?.todayCount ?? '—'}</p>}
            </div>
          </div>
          ...
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-200 flex flex-col gap-3">
            ...
            <div>
              ...
              {loading ? <Skeleton className="h-8 w-20 mt-1" /> : <p className="text-slate-900 text-2xl font-bold mt-1">{orderStats ? orderStats.revenue.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '—'} MAD</p>}
            </div>
          </div>
        </div>
                ...
                <tbody className="divide-y divide-slate-200">
                  {recentOrders.map((order) => (
                    <tr key={order.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 text-sm font-semibold text-slate-900">#{String(order.id).padStart(4, '0')}</td>
                      ...
                      <td className="px-6 py-4 text-sm font-bold text-slate-900">{Number(order.totalPrice).toFixed(2)} MAD</td>

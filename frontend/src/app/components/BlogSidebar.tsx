...
export default function BlogListingSidebar({ 
    ...
    return (
        <aside className="w-full lg:w-[360px] shrink-0 space-y-8 lg:sticky lg:top-28">
            ...
            {toc && toc.length > 0 && (
                <div className="bg-[#111] rounded-[24px] p-8 border border-white/5 transition-all hover:border-white/10">
                    <div className="flex items-center gap-3 mb-6">
                        <div className="w-1 h-6 rounded-full bg-[#99cc00]" />
                    ...
                    <nav className="space-y-1">
                        {toc.map((item, idx) => (
                            <a 
                                key={idx}
                                href={`#${item.id}`}
                                className="block py-2 text-[14px] font-bold text-slate-500 hover:text-[#99cc00] hover:translate-x-1 transition-all"
                            >
                ...
                <div className="bg-[#111] rounded-[24px] p-8 border border-white/5 transition-all hover:border-white/10">
                    <div className="flex flex-col items-center text-center">
                        <div className="relative mb-6">
                            <div className="size-24 rounded-full bg-white/5 p-1.5 border border-white/10">
                                <div className="size-full rounded-full bg-[#99cc00] flex items-center justify-center text-black text-3xl font-black uppercase">
                                    {author.name.charAt(0)}
                                </div>
                            </div>
                            <div className="absolute -bottom-1 -right-1 size-8 rounded-full bg-white border border-slate-200 flex items-center justify-center text-[#99cc00] shadow-sm">
                                <span className="material-symbols-outlined text-[16px] font-black">verified</span>
                            </div>
                        </div>
                        <h4 className="text-[18px] font-black text-white uppercase tracking-tight mb-1">{author.name}</h4>
                        <span className="text-[11px] font-black text-[#99cc00] uppercase tracking-[0.2em] mb-4">{author.role}</span>
            ...
            <div className={`rounded-[32px] overflow-hidden border transition-all hover:border-white/10 group ${
                variant === 'dark' 
                ? 'bg-black border-white/5 p-10 pt-12 pb-12' 
                : 'bg-[#111] border-white/5'
            }`}>
                {/* Visual Header Block — Only for Light Variant */}
                {variant === 'light' && (
                    <div className="h-32 bg-gradient-to-br from-[#99cc00] to-[#b4f500] relative overflow-hidden">
                ...
                <div className={`${variant === 'light' ? 'p-8 pt-10' : ''} text-center`}>
                    ...
                    <p className={`text-[14px] font-medium leading-relaxed mb-10 ${
                        ...
                        <span className={`${variant === 'dark' ? 'text-white/40' : 'text-slate-400 font-bold'}`}>
                            {variant === 'dark' ? '+12 000 abonnés.' : 'Rejoignez 12 000+ passionnés.'}
                        </span>
                    </p>
                    ...
                    <form onSubmit={handleNewsletterSubmit} className="space-y-4">
                        <div className="relative group/input">
                            <input
                                className={`w-full rounded-2xl border px-6 py-4.5 text-[15px] font-bold outline-none transition-all text-center ${
                                    variant === 'dark'
                                    ? 'bg-white/5 border-white/10 text-white placeholder-white/20 focus:bg-white/10 focus:ring-4 focus:ring-[#99cc00]/20 focus:border-[#99cc00]/40'
                                    : 'bg-white/5 border-white/10 text-white placeholder-white/20 focus:bg-white/10 focus:ring-4 focus:ring-[#99cc00]/20 focus:border-[#99cc00]/40'
                                }`}
                        ...
                        <button
                            type="submit"
                            disabled={newsletterStatus === 'loading'}
                            className="w-full rounded-2xl bg-[#99cc00] py-4.5 text-[13px] font-black text-black uppercase tracking-[0.2em] transition-all duration-300 hover:bg-[#b4f500] hover:scale-[1.02] shadow-xl shadow-[#99cc00]/20 active:scale-[0.98] flex items-center justify-center gap-2 disabled:opacity-60"
                        >
                            {newsletterStatus === 'loading' ? (
                                <div className="h-5 w-5 border-[3px] border-black border-t-transparent rounded-full animate-spin" />
                            ) : (
                                    ...
                                    {visibleTags.map((t, idx) => (
                                        <button
                                            ...
                                            className={`px-4 py-2 rounded-full text-[13px] font-semibold transition-all border ${
                                                activeTag === t 
                                                    ? 'bg-[#99cc00] text-black border-[#99cc00] shadow-lg shadow-[#99cc00]/15' 
                                                    : 'bg-white/5 text-slate-500 hover:bg-[#99cc00]/5 hover:text-[#99cc00] border-white/10 hover:border-[#99cc00]/15'
                                            }`}
                                        >
                                            #{t}
                                        </button>
            ...
            {recentPosts.length > 0 && (
                <div className="bg-[#111] rounded-[24px] p-8 border border-white/5 transition-all hover:border-white/10">
                    <h4 className="text-[15px] font-black text-white uppercase tracking-wider mb-6">
                        Recent Posts
                    </h4>
                    <div className="space-y-5">
                        {recentPosts.map((post) => (
                            ...
                            >
                                <div className="w-[72px] h-[72px] rounded-md overflow-hidden shrink-0 bg-white/5 border border-white/10">
                                    {post.imageUrl ? (
                                        <img
                                ...
                                <div className="flex-1 min-w-0">
                                    <h5 className="text-[14px] font-bold text-white leading-snug line-clamp-2 group-hover:text-[#99cc00] transition-colors">
                                        {post.title}
                                    </h5>
                                    <span className="text-[11px] font-semibold text-slate-500 uppercase tracking-wider mt-1.5 block">
            ...
            {activeTip && !hideTip && (
                <div className="bg-[#111] rounded-[32px] p-9 text-white shadow-2xl relative overflow-hidden group border border-white/5">
                    <div className="absolute top-0 right-0 w-40 h-40 bg-[#99cc00] opacity-[0.06] rounded-full translate-x-[20%] -translate-y-[20%] blur-3xl group-hover:opacity-[0.12] transition-opacity" />
                    
                    <div className="relative z-10">
                        <span className="text-[13px] font-black uppercase tracking-[0.3em] text-[#99cc00] mb-6 block drop-shadow-[0_0_10px_rgba(153,204,0,0.3)]">
                            Astuce du Moment
                        </span>
                        ...
                        <div className="border-t border-white/10 pt-6">
                            <p className="text-[14px] font-medium text-white/40">
                                Par <span className="text-[#99cc00] font-bold">{activeTip.authorName},</span>{' '}

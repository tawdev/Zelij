...
export class OrdersService {
    ...
    async findByReference(reference: string): Promise<Order> {
        const order = await this.orderRepository.findOne({ where: { invoiceReference: reference } });
        if (!order) {
            throw new NotFoundException(`Commande avec la référence ${reference} introuvable.`);
        }
        return order;
    }
    ...
    async updateStatus(id: number, status: OrderStatus, email?: string): Promise<Order> {
        ...
        // TRIGGER AUTOMATED EMAIL: Notifies the customer about the status change
        if (oldStatus !== status) {
            console.log(`[DEBUG] Changement de statut détecté pour #${savedOrder.id}: ${oldStatus} -> ${status}`);
            await this.mailService.sendOrderStatusUpdate(savedOrder);
        }
    ...
    async resendInvoice(id: number): Promise<{ success: boolean; message: string }> {
        ...
        return { success: true, message: 'Devis renvoyé avec succès.' };
    }
